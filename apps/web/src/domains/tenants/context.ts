...
import { z } from "zod";
import { getCurrentUser, type CurrentUser } from "@/domains/auth/session";
...
export const getTenantContext = cache(async (): Promise<TenantContext | null> => {
  ...
  const { data: permissionRows } = await supabase.rpc("get_my_permissions", { p_tenant_id: active.summary.id });
...
export async function requireTenantContext(): Promise<TenantContext> {
  ...
  if (!context) redirect(ROUTES.onboarding);
  return context;
}
