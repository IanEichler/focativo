...
import { aiBusinessInfoSchema, aiSettingsSchema, type AiBusinessInfoField, type AiSettingsField } from "./schemas";
...
export async function saveAiBusinessInfoAction(
  _prev: ActionState<AiBusinessInfoField>,
  formData: FormData,
): Promise<ActionState<AiBusinessInfoField>> {
  const context = await requireTenantContext();
  if (!context.can("tenant.update")) return { status: "error", message: toUserMessage({ message: "forbidden" }) };

  const input = formDataToObject(formData);
  const parsed = aiBusinessInfoSchema.safeParse(input);
  if (!parsed.success) return validationError(parsed.error, input);
  const { businessDescription, generalPolicies, faq, screeningFlow } = parsed.data;

  const supabase = await createClient();
  const { error } = await supabase.rpc("ai_business_info_update", {
    p_tenant_id: context.tenant.id,
    p_business_description: businessDescription,
    p_general_policies: generalPolicies,
    p_faq: JSON.stringify(faq),
    p_screening_flow: JSON.stringify(screeningFlow),
  });
  if (error) return { status: "error", message: toUserMessage(error) };

  revalidatePath("/app/ia");
  return { status: "success", message: "Conhecimento de negócio salvo." };
}
